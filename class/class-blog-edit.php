...
class Blog_ArticleEdit {
    ...
    public static function get_instance() {
        ...
        if ( null == self::$instance ) {
            self::$instance = new Blog_ArticleEdit();
        } 
    ...
    private function __construct() {
        ...
        $this->templates = array(
            '../templates/blogging-blog-edit.php' => 'Blogging Blog Edit',
        );
    ...
    public function make_article_edit_page () {
        ...
        if ( null == get_page_by_title( $title ) && get_current_blog_id() == 1 ) {
            ...
            $uploader_page = array(
                ...
                'hierarchical '         => true,
                'page_template'         => '../templates/blogging-blog-edit.php'
            );
            ...
            if ( !$post_id ) {
                ...
                update_post_meta( $post_id, '_wp_page_template', '../templates/blogging-blog-edit.php' );
    ...
    public function save_blog () {
        ...
        if ( $_POST['post_id'] ) {
            ...
            update_post_meta($postId, 'subtitle', self::clean_input( $_POST['subtitle'] ) );
        ...
        $result = array(
            'status'    => $_POST['status'],
            'redirect'  => '/doctor-platform/blog-edit?post_id=' . $postId,
            'preview'  =>  '/doctor-platform/preview/?post=' . $postId . '&preview=true',
            'goback'    => '/doctor-platform/my-blogs',
            'message'   => ( $_POST['status'] == 'draft' ) ? 'Saved Blog!' : 'Published Blog!' 
        );
     ...
     * Save Articles in DB
     */
    public function save_link_blog () {
        ...
        if ( $_POST['post_id'] ) {
            ...
            $result = array(
                'status'    => $_POST['status'],
                'redirect'  => '/doctor-platform/my-blogs',
                'message'   => 'Blog Pending Approval!' 
            );
    ...
    public function delete_article () {
        ...
        if ( $_POST['status'] == 'delete') {
            ...
            wp_send_json_success( 'Blog Deleted' );
...
add_action( 'wp_loaded', array( 'Blog_ArticleEdit', 'get_instance' ) );

add_action('wp_ajax_nopriv_choose_random_image', array( 'Blog_ArticleEdit', 'choose_random_image' ) );

add_action('wp_ajax_choose_random_image', array( 'Blog_ArticleEdit', 'choose_random_image' ) );

add_action('wp_ajax_nopriv_save_blog', array( 'Blog_ArticleEdit', 'save_blog' ) );

add_action('wp_ajax_save_blog', array( 'Blog_ArticleEdit', 'save_blog' ) );

add_action('wp_ajax_nopriv_save_link_blog', array( 'Blog_ArticleEdit', 'save_link_blog' ) );

add_action('wp_ajax_save_link_blog', array( 'Blog_ArticleEdit', 'save_link_blog' ) );

add_action('wp_ajax_nopriv_preview_article', array( 'Blog_ArticleEdit', 'preview_article' ) );

add_action('wp_ajax_preview_article', array( 'Blog_ArticleEdit', 'preview_article' ) );

add_action('wp_ajax_nopriv_delete_article', array( 'Blog_ArticleEdit', 'delete_article' ) );

add_action('wp_ajax_delete_article', array( 'Blog_ArticleEdit', 'delete_article' ) );